...
export function PanelsLayout({ panels, options }: Props): React.ReactElement | null {
  const width = useWindowWidth(10);
  const pages = useMemo(() => solvePanelsPages(panels, width, options), [panels, width, options]);
  ...
  const pageIndexFixed = useMemo(() => {
    if (arrayShallowEqual(prevPanelsKeys.panelsKeys, panelsKeys.panelsKeys) === false) {
...
function usePrevious<T>(value: T, equal: (l: T, r: T) => boolean = refEqual): T {
