...
export function solvePanelsPages(
  ...
  const maxFirstPanelIndex = solveMaxFirstPanelIndex(panelsFull, width, spacing);
...
function solvePanelsPage(
  ...
  const backFilled = fillPanels(displayed, panels.slice(0, firstIndex).reverse(), width, spacing);
  ...
  const flexValues = displayed.map((p) => (p.flex === 0 ? (hasFlex ? 0 : 1) : p.flex));
  const flexSum = flexValues.reduce((acc, v) => acc + v, 0);
  // found how many pixel to add to each panel (rounded down to exact piwel)
  const flexSizes = flexValues.map((flex) => Math.floor((rest / flexSum) * flex));
...
function fillPanels<P extends Panel>(
  ...
  while (queue.length > 0) {
    ...
    const nextWidth = sumWithSpacing([...displayedPanelsWidth, panel.width], spacing);
    if (displayedPanels.length === 0 || nextWidth < width) {
      ...
      const hasOptionalDisplayed = displayedPanels.filter((p) => p.optional === true);
...
function sumWithSpacing(sizes: Array<number>, spacing: Spacing): number {
  ...
  return sizesSum + spacing.spaceAround * 2 + (sizes.length - 1) * spacing.spaceBetween;
}
