...
function grid(big: number, half: 0 | 1 = 0, quarter: 0 | 1 = 0, eighth: 0 | 1 = 0): number {
          ...
          ZONES.map((zone) => {
            return (
              <Zone
                ...
                multiline={multiline}
                onDiceClick={(dice) => {
                  setItems((prev) => [...prev, { zone: zone.multiply, value: dice }]);
                }}
              />
            );
          }),
...
function Zone({ color, multiply, items, onDiceClick, multiline }: ZoneProps) {
  return (
    ...
    >
      ...
      >
        {DICES.map((dice, i) => {
          const count = items.filter((v) => v.zone === multiply && v.value === dice.value).length;
          return (
            <Dice
              dice={dice}
              key={i}
...
function addBetween<T>(list: Array<T | null>, addItem: (sepIndex: number, before: T, after: T) => T): Array<T> {
